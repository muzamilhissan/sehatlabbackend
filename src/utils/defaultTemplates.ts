...
export const defaultTemplates = [
  ...
  },
  {
    ...
    category: 'Serology',
    parameters: [
      { name: 'Dengue NS1', unit: 'Index', min: 0, max: 0.9 },
    ]
  },
  {
    ...
    parameters: [
      { name: 'Malaria Antigen', unit: '', min: null, max: null },
    ]
  },
  {
    name: 'Widal Test',
...
export async function copyDefaultTemplatesForUser(userId: number) {
  try {
    const count = await prisma.testTemplate.count({ where: { userId } });
    if (count > 0) {
      console.log(`User ${userId} already has ${count} templates.`);
      return;
    }
    console.log(`Copying default templates for User ${userId}...`);
    ...
    console.log(`Successfully provisioned default templates for User ${userId}`);
  } catch (error) {
    console.error(`Failed to copy default templates for User ${userId}:`, error);
  } finally {
